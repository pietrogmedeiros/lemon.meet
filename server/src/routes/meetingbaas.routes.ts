...
router.post('/webhook', async (req: Request, res: Response) => {
  ...
  setImmediate(async () => {
    try {
      ...
      // Eventos calendar.* do MeetingBaas ignorados de propósito: o calendário é
      // tratado 100% pelo CalendarCronService (Google direto → BotRouter). A
      // integração nativa de calendário do MeetingBaas não é usada (nenhuma
      // calendar_integration tem baas_calendar_id).
    } catch (err) {
