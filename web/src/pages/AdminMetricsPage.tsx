...
function ChartTooltip({ hover, containerWidth, n }: {
  ...
  return (
    <div
      className="absolute pointer-events-none z-10 bg-surface rounded-md shadow-lg border border-neutral-light px-2.5 py-1.5 text-xs"
      style={{ left, top: -8, transform: 'translateY(-100%)', minWidth: 120, maxWidth: tipW }}
    >
      <div className="font-medium text-primary mb-1">{hover.title}</div>
      {hover.lines.map((l, idx) => (
        <div key={idx} className="flex items-center justify-between gap-3">
          ...
          <span className="tabular-nums text-primary font-medium">{l.value}</span>
        </div>
      ))}
    </div>
  )
...
function KpiCard({ title, value, hint }: { title: string; value: string; hint?: string }) {
  return (
    <div className="rounded-lg border border-neutral-light bg-surface p-4">
      <div className="text-[11px] uppercase tracking-wider text-secondary">{title}</div>
      ...
      <MainLayout>
        <div className="p-6">
          <Card>
            ...
            <p className="text-sm text-secondary">Este painel é apenas para devs autorizados.</p>
    ...
    <MainLayout>
      <div className="p-4 sm:p-6 max-w-7xl">
        <div className="flex flex-wrap items-end justify-between gap-3 mb-4">
          <div>
            <h1 className="text-2xl font-semibold text-primary">Métricas internas</h1>
          ...
          <div className="flex items-center gap-2">
            <select
              ...
              className="px-3 py-1.5 border border-neutral-light rounded text-sm bg-surface"
            >
            ...
            <button
              ...
              disabled={loading}
              className="px-3 py-1.5 border border-neutral-light rounded text-sm bg-surface hover:bg-background disabled:opacity-50"
            >
          ...
          <>
            ...
            <Section title="Crescimento" subtitle="Signups, ativação e usuários ativos por dia">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div>
                  <div className="text-xs text-secondary mb-1">Signups/dia</div>
                  <BarChart labels={tsLabels} values={data.timeseries.signupsByDay} color="#5A7F47" />
                  <div className="text-xs text-tertiary mt-1">{data.timeseries.signupsByDay.reduce((a, b) => a + b, 0)} no período</div>
                </div>
                <div>
                  <div className="text-xs text-secondary mb-1">Usuários ativos/dia</div>
                  <BarChart labels={tsLabels} values={data.timeseries.activeUsersByDay} color="#2D5A27" />
                  <div className="text-xs text-tertiary mt-1">pico: {Math.max(...data.timeseries.activeUsersByDay, 0)}</div>
                </div>
                <div>
                  <div className="text-xs text-secondary mb-1">Ativação</div>
                  <div className="text-3xl font-semibold text-brand">{formatPct(data.overview.activationRate)}</div>
                  <div className="text-xs text-tertiary">{data.overview.usersWithMeeting} de {data.overview.totalUsers} users já fizeram ≥1 meeting</div>
                </div>
              </div>
            </Section>
            ...
            <Section title="Engajamento" subtitle="Meetings por dia, por fonte e por plataforma">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-5">
                <div>
                  <div className="text-xs text-secondary mb-1">Distribuição de duração</div>
                  <div className="space-y-1">
                    {data.quality.durationBuckets.map(b => {
                      const max = Math.max(...data.quality.durationBuckets.map(x => x.count), 1)
                      return (
                        <div key={b.label} className="flex items-center gap-2 text-xs">
                          <div className="w-16 text-secondary">{b.label}</div>
                          <div className="flex-1 bg-neutral-lighter rounded h-4 relative overflow-hidden">
                  ...
                  <div className="text-xs text-tertiary mt-2">média: {formatDuration(data.quality.avgDurationSeconds)}</div>
                </div>
                <div>
                  <div className="text-xs text-secondary mb-1">Meetings/dia (total)</div>
                  <BarChart labels={tsLabels} values={data.timeseries.meetingsByDay} color="#2D5A27" height={140} />
                  <div className="text-xs text-tertiary mt-1">total: {data.timeseries.meetingsByDay.reduce((a, b) => a + b, 0)}</div>
                </div>
              </div>
            </Section>
            ...
            <Section title="Qualidade do core" subtitle="Funil de transcrição → insights e razões de falha">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="col-span-1">
                  <div className="text-xs text-secondary mb-2">Funil ({data.range === 'all' ? 'tudo' : `${data.rangeDays}d`})</div>
                  {(() => {
                        ...
                        {rows.map(r => (
                          <div key={r.label}>
                            <div className="flex justify-between text-xs mb-0.5">
                              ...
                              <span className="tabular-nums text-secondary">{r.count} ({formatPct((r.count / max) * 100)})</span>
                            </div>
                            <div className="bg-neutral-lighter rounded h-3 overflow-hidden">
                ...
                <div className="col-span-1 lg:col-span-2">
                  <div className="text-xs text-secondary mb-2">Falhas por motivo</div>
                  {data.quality.failureBreakdown.length === 0 ? (
                    <p className="text-sm text-tertiary">Sem falhas no período. 🎉</p>
                  ) : (
                    <div className="space-y-1">
                      {data.quality.failureBreakdown.map(f => {
                        ...
                        return (
                          <div key={f.reason} className="flex items-center gap-2 text-xs">
                            ...
                            <div className="flex-1 bg-neutral-lighter rounded h-4 overflow-hidden">
                              <div className="h-full" style={{ width: `${(f.count / max) * 100}%`, background: colorForReason(f.reason) }} />
                            </div>
                            <div className="w-16 text-right tabular-nums text-secondary">{f.count} · {formatPct(f.pct)}</div>
                          </div>
                        )
                      })}
                    </div>
              ...
              <div className="mt-5">
                <div className="text-xs text-secondary mb-1">Falhas/dia stacked por motivo</div>
                <StackedBarChart
                  labels={tsLabels}
                  ...
                  {data.adoption.usersBySource.map(s => {
                    ...
                    return (
                      <div key={s.source} className="flex items-center gap-2 text-xs">
                        ...
                        <div className="w-28 text-right tabular-nums text-secondary">{s.users} ({formatPct(s.pct)})</div>
                      </div>
                    )
                  })}
                </div>
            ...
            <Section title="Times" subtitle="Top 10 por atividade e distribuição">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {data.teams.teams.slice(0, 10).map(t => (
                      <tr key={t.id} className="border-b border-neutral-light">
              ...
              <div className="grid grid-cols-2 gap-4 mt-4">
                <div>
                  <div className="text-xs text-secondary mb-1">Tipo de time</div>
                  <div className="space-y-1">
                ...
                <div>
                  <div className="text-xs text-secondary mb-1">Framework de avaliação</div>
                  <div className="space-y-1">
            ...
            <Section title="Retenção (cohorts semanais)" subtitle="% de usuários da cohort que voltaram a ter meeting na semana N">
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="text-secondary border-b border-neutral-light">
                  ...
                  <tbody>
                    {data.retention.cohorts.map(c => (
                      <tr key={c.weekLabel} className="border-b border-[#F8F9FA]">
                        ...
                        <td className="py-1.5 px-2 text-right text-secondary tabular-nums">{c.size}</td>
                        {c.retention.map((v, w) => (
                          <td
                            key={w}
              ...
              {data.retention.cohorts.length === 0 && <p className="text-sm text-tertiary">Sem dados suficientes ainda.</p>}
            </Section>
            ...
            <Section title="Uso de IA" subtitle="Chats, tokens e reprocessamentos por dia">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div>
                  ...
                  <div className="text-xs text-tertiary mt-1">total: {data.aiUsage.totalChats}</div>
                </div>
                <div>
                  <div className="text-xs text-secondary mb-1">Tokens IA/dia</div>
                  <BarChart labels={tsLabels} values={data.aiUsage.tokensByDay} color="#9C8EE0" />
                  <div className="text-xs text-tertiary mt-1">total: {formatNumber(data.aiUsage.totalTokens)}</div>
                </div>
                <div>
                  <div className="text-xs text-secondary mb-1">Reprocessamentos/dia</div>
                  <BarChart labels={tsLabels} values={data.aiUsage.reprocessByDay} color="#A47A47" />
                  <div className="text-xs text-tertiary mt-1">sinal de qualidade ruim de insights</div>
                </div>
              </div>
            </Section>
            ...
            <Section title="Voz do usuário" subtitle="Últimos 10 feedbacks coletados">
              {data.feedback.length === 0 ? (
                <p className="text-sm text-tertiary">Sem feedbacks ainda.</p>
              ) : (
                <div className="space-y-3">
                  {data.feedback.map(f => (
                    <div key={f.id} className="border-l-2 border-[#5A7F47] pl-3 text-sm">
                      ...
                      {f.howUsing && <div><span className="text-tertiary text-xs">como usa:</span> {f.howUsing}</div>}
                      {f.whatThink && <div><span className="text-tertiary text-xs">o que acha:</span> {f.whatThink}</div>}
                      {f.featureRequest && <div><span className="text-tertiary text-xs">pediu:</span> {f.featureRequest}</div>}
                    </div>
                  ))}
                </div>
              )}
